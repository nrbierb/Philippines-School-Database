...
class StudentsClass(object):
    ...
    def __init__(self, class_session, name):
        ...
        self.grade_values = []
        self.grade_element_ids = []
        self.grade_instance_ids = []
    ...
    def set_grade(self, element_id, instance_id,
                  grade):
        """
        Set the grade information across the three lists.
        Search the element and instance lists for a previous
        entry to just update if it exists. It not, as is the
        probable case, add the information to the end of all
        three lists.
        """
        index = self.get_grade_index(element_id, instance_id)
        if (index != -1):
            #update a prior grade
            self.grade_values[index] = grade
        else:
            self.grade_element_ids.append(element_id)
            self.grade_instance_ids.append(instance_id)
            self.grade_values.append(grade)
        
    def get_grade_index(self, element_id, instance_id):
        """
        Search the element and instance lists for an entry with
        both values as defined. The tuple element_id, instance_id
        uniquely defines an entry. A return value of -1 indicates
        that no match has been found
        """
        index = -1
        for i in xrange(0, len(self.grade_element_ids)):
            if ((self.grade_element_ids[i] == element_id) and
                (self.grade_instance_ids[i] == instance_id)):
                index = i
                break
        return index
    ...
    def get_grades_for_grading_element(self, grading_element):
        ...
        element_grades = []
        for i in xrange(0, len(self.grade_element_ids)):
            if (self.grade_element_ids[i] == grading_element_id):
                element_grades.append(self.grade_values[i])
        return element_grades
...
class ClassSession(object):
    """
    This represents what could be called a "scheduled class", a class
...
class ClassGradingElement(object):
    ...
    def get_instance_date(self, instance_id):
        if (instance_id <= len(self.dates)):
            return self.date[instance_id]
        else:
            return None
    
    def change_date(self, instance_id, new_date):
        if (instance_id <= len(self.dates)):
            self.date[instance_id] = new_date
